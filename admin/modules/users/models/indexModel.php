...
function old_password_exists($password_old) {
    $check_old_password = db_num_rows("SELECT * FROM `tbl_users` WHERE `password` = '{$password_old}'");
...
function update_password_new($username, $data) {
    db_update('tbl_users', $data, "`username` = '{$username}'");
}
